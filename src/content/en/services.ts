import type { ServicesContent } from "../types";

export const services: ServicesContent = {
  title: "Coaching & Services",
  services: [
    {
      name: "Executive & Leadership Coaching",
      description:
        "For leaders and managers seeking clarity, impact, and coherence.",
      formats: [
        "8 to 12 sessions (in person / video) — for complex transitions, exploring a new role, or negotiating a position or departure.",
        "2 to 4 sessions (in person / video) — to prepare for a key conversation or a one-off interview.",
      ],
      outcome: "Clearer, faster, more aligned.",
    },
    {
      name: "Transitions & Negotiation",
      description:
        "For sensitive repositioning, departures, or major turning points. Clarifying objectives, negotiation preparation, legal and HR insight, emotional management, repositioning strategy, narrative & visibility.",
      formats: [],
      outcome: "Emerge stronger, clearer, and fully in command.",
    },
  ],
};
