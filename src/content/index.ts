...
export async function getAboutContent(locale: Locale): Promise<AboutContent> {
  const data = await client.fetch(
    `*[_type == "aboutPage"][0]{
      ...
      "intro": intro[$locale],
      "bio": bio[$locale],
      "singularityIntro": singularityIntro[$locale],
      "singularityItems": singularityItems[]${LOCALIZED_ARRAY_ITEM},
      "singularityConclusion": singularityConclusion[$locale],
  ...
  return {
    ...
    bio: data.bio,
    singularity: {
      intro: data.singularityIntro,
      items: data.singularityItems ?? [],
      conclusion: data.singularityConclusion,
    },
